...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = useCallback((vehicle, options = {}) => {
    // Vérifier l'authentification avant d'ajouter au panier
    const token = localStorage.getItem('token');
    if (!token) {
      // Déclencher un événement pour demander la connexion
      window.dispatchEvent(new CustomEvent('auth-required', {
        detail: { vehicle, options, returnUrl: window.location.pathname }
      }));
      return false;
    }
