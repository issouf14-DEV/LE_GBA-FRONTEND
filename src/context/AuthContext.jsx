...
export const AuthProvider = ({ children }) => {
  ...
  const fetchUser = async () => {
    try {
      ...
      // Appeler le endpoint /api/auth/profile pour récupérer les données utilisateur
      try {
        ...
        if (userData) {
          ...
          console.log('✅ Utilisateur chargé depuis /api/auth/profile');
        }
      } catch (apiError) {
        console.warn('⚠️ Erreur appel /api/auth/profile, fallback localStorage:', apiError.message);
        // Fallback sur localStorage si le endpoint échoue
        const savedUser = localStorage.getItem('user');
        if (savedUser) {
          try {
            const userData = JSON.parse(savedUser);
