...
export default function ManageOrders() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      // Le backend retourne { orders, page, totalPages, total } ou un tableau directement
      const responseData = response.data || {};
      const rawServerOrders = Array.isArray(responseData) ? responseData : (responseData.orders || []);
  ...
  useEffect(() => {
    const calculateGlobalStats = async () => {
      try {
        ...
        const statsRawOrders = Array.isArray(statsData) ? statsData : (statsData.orders || []);
        const serverOrders = statsRawOrders.map(o => normalizeOrder(o, false));
